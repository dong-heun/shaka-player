...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async attach(mediaElement, initializeMediaSource = true) {
    ...
    try {
      ...
      if (initializeMediaSource && device.supportsMediaSource() &&
        ...
        // Don't initialize MediaSourceEngine here if we don't have manifest yet.
        // It will be initialized in load() after manifest is parsed.
        // This allows platform-specific setup (e.g., WebOS mediaOption) to be
        // done before MediaSource creation.
        if (this.manifest_) {
    ...
    try {
      ...
      if (shouldUseSrcEquals) {
        ...
        await mutexWrapOperation(async () => {
          ...
          // Set platform-specific mediaOption from manifest (e.g., WebOS)
          // WebOS optimizes resource allocation when resolution/framerate hints
          // are provided before MediaSource creation. This prevents unnecessary
          // resource cleanup and enables efficient adaptive streaming.
          // This handles cases where MediaSourceEngine was created before
          // manifest was available (e.g., attach() -> load() scenario).
          if (this.manifest_ && this.video_) {
            const device = shaka.device.DeviceFactory.getDevice();
            if (device.getDeviceName() === 'WebOS') {
              shaka.log.info(
                  'WebOS: Setting mediaOption after manifest parsed');
              device.setMediaOptionFromManifest(this.video_, this.manifest_);
            }
          }
        }, 'waitForFinish');
  ...
  async initializeMediaSourceEngineInner_() {
    ...
    // Set platform-specific mediaOption from manifest (e.g., WebOS)
    // WebOS uses this hint to optimize resource allocation for adaptive
    // streaming, preventing over-allocation and minimizing resource conflicts.
    // MUST be done BEFORE createMediaSourceEngine, which creates MediaSource
    // in its constructor and sets video.src.
    // This handles both attach() and load() scenarios.
    shaka.log.info('initializeMediaSourceEngineInner: device=' +
        device.getDeviceName() + ', manifest=' + !!this.manifest_ +
        ', video=' + !!this.video_);
    if (this.manifest_ && this.video_) {
      if (device.getDeviceName() === 'WebOS') {
        shaka.log.info('WebOS: Calling setMediaOptionFromManifest');
        device.setMediaOptionFromManifest(this.video_, this.manifest_);
      }
    }
